...
class ModelTrainer:
    ...
    def initialize(self, config):
        ...
        if "user_architecture" not in config or "item_architecture" not in config:
            raise ValueError(
                "Config must contain user_architecture and item_architecture"
            )
        ...
        self.optimizer = optim.Adam(
            [{"params": user_params, "lr": lr}, {"params": item_params, "lr": lr}],
    ...
    def train(self, dataset, epochs=10):
        ...
        # Training history
        history = {"loss": [], "rating_loss": [], "diversity_loss": [], "val_loss": []}
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(indices), batch_size):
                ...
                user_batch = torch.tensor(
                    dataset["user_features"][batch_indices], dtype=torch.float32
                ).to(self.device)
                item_batch = torch.tensor(
                    dataset["item_features"][batch_indices], dtype=torch.float32
                ).to(self.device)
                rating_batch = torch.tensor(
                    dataset["ratings"][batch_indices], dtype=torch.float32
                ).to(self.device)
                ...
                diversity_loss = torch.mean(
                    torch.pow(user_sim * (1 - identity), 2)
            ...
            with torch.no_grad():
                ...
                val_indices = np.random.choice(
                    indices, min(1000, len(indices)), replace=False
                )

                val_user = torch.tensor(
                    dataset["user_features"][val_indices], dtype=torch.float32
                ).to(self.device)
                val_item = torch.tensor(
                    dataset["item_features"][val_indices], dtype=torch.float32
                ).to(self.device)
                val_rating = torch.tensor(
                    dataset["ratings"][val_indices], dtype=torch.float32
                ).to(self.device)
                ...
                val_pred_ratings = torch.sum(
                    val_user_embeddings * val_item_embeddings, dim=1
                )
    ...
    def evaluate(self, dataset):
        ...
        with torch.no_grad():
            user_test = torch.tensor(dataset["user_features"], dtype=torch.float32).to(
                self.device
            )
            item_test = torch.tensor(dataset["item_features"], dtype=torch.float32).to(
                self.device
            )
            rating_test = torch.tensor(dataset["ratings"], dtype=torch.float32).to(
                self.device
            )
            ...
            print(
                f"Test User Emb Var: {user_emb_var:.6f}, Item Emb Var: {item_emb_var:.6f}"
            )
