...
class Recommender:
    ...
    def get_recommendations(self, user, num_results=DEFAULT_NUM_RECOMMENDATIONS):
        ...
        if (
            ...
            and isinstance(user, (list, np.ndarray))
            and len(np.array(user).shape) == 1
        ):
        ...
        candidate_embeddings, candidate_ids = ann_search.ann_search(
            self.ann_index, user_embedding
        )
        ...
        refined_results = sorted(refined_results, key=lambda x: x[1], reverse=True)[
            :num_results
        ]
        ...
        for item_id, score in refined_results:
            rec = {"item_id": item_id, "score": score}
