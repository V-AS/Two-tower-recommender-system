...
class ANNSearch:
    ...
    def build_index(self, embeddings, item_ids, index_type=DEFAULT_INDEX_TYPE):
        ...
        try:
            ...
            print(
                f"Embedding stats - min: {embeddings.min():.6f}, max: {embeddings.max():.6f}"
            )
            print(
                f"Embedding std: {embeddings.std():.6f}, mean: {embeddings.mean():.6f}"
            )
            ...
            if index_type == "Flat":
                ...
                nlist = min(
                    int(np.sqrt(len(embeddings)) * 4), len(embeddings) // 10 or 1
                )
                ...
                index.nprobe = min(
                    nlist // 4 + 1, 16
                )  # Higher nprobe for better accuracy
                print(f"Set nprobe to {index.nprobe}")
            else:
                raise ValueError(f"Unsupported index type: {index_type}")

            # Create the ANN index object
            ann_index = {"index": index, "item_ids": item_ids, "index_type": index_type}
    ...
    def ann_search(self, index, query, candidates=100):
        ...
        if (
            ...
            or "index" not in index
            or "item_ids" not in index
        ):
    ...
    def load_index(self, path):
        ...
        try:
            ...
            if not os.path.exists(f"{path}.faiss") or not os.path.exists(
                f"{path}.meta.npy"
            ):
