...
class DataProcessor:
    ...
    def preprocess_data(self, df):
        ...
        try:
            ...
            data["Year-Of-Publication"] = data["Year-Of-Publication"].fillna(
                pub_year_median
            )
        except Exception:
        ...
        data["Author-Frequency"] = (
            data["Author-Popularity"] / data["Author-Popularity"].max()
        )
        data["Publisher-Frequency"] = (
            data["Publisher-Popularity"] / data["Publisher-Popularity"].max()
        )
        ...
        if decade_min == decade_max:
            ...
            data["Decade-Normalized"] = (data["Decade"] - decade_min) / (
        ...
        if year_min == year_max:
            ...
            data["Year-Normalized"] = (data["Year-Of-Publication"] - year_min) / (
                year_max - year_min
            )
        ...
        labels = [0, 1, 2, 3, 4, 5]
        try:
            data["Age-Group"] = pd.cut(data["Age"], bins=bins, labels=labels).astype(
                float
            )
