    ...
    def initialize(self, user_model, item_model):
        ...
        try:
            ...
            if user_out.shape[1] != item_out.shape[1]:
                raise ValueError(
                    "User and item models must produce embeddings of the same dimension"
                )
        ...
        with torch.no_grad():
            ...
            if len(users) == 1:  # Only apply to single user inference (recommendation)
                ...
                for i in range(num_variations):
                    ...
                    perturbed_input = (
                        users_tensor + torch.randn_like(users_tensor) * noise_scale
                    )
    ...
    def generate_item_embedding(self, items):
        ...
        with torch.no_grad():
            for i in range(0, len(items), batch_size):
                ...
                normalized_batch_embeddings = torch.nn.functional.normalize(
