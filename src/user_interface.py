...
def initialize_system():
    ...
    try:
        ...
        debug_print(f"Loaded dataset with {len(data)} records")

        # Check available columns
        debug_print(f"Available columns: {data.columns.tolist()}")
        ...
        state_freq = {
            state.strip().lower(): count / len(processed_data)
            for state, count in state_counts.items()
        }
        country_freq = {
            country.strip().lower(): count / len(processed_data)
        ...
        # Define age groups
        age_bins = [0, 18, 25, 35, 50, 100, 300]
    ...
    try:
        ...
        user_state_dict = load_model(os.path.join(OUTPUT_DIR, "user_model.pth"))
        item_state_dict = load_model(os.path.join(OUTPUT_DIR, "item_model.pth"))

        # Create model instances with the correct architecture
        # For user features: Age-Normalized, Age-Group, State-Frequency, Country-Frequency
        # For item features: Year-Normalized, Author-Frequency, Publisher-Frequency, Decade
        user_model = create_user_tower(input_dim=4, embedding_dim=32)
        item_model = create_item_tower(input_dim=4, embedding_dim=32)
        ...
        book_mapping = np.load(
            os.path.join(OUTPUT_DIR, "book_mapping.npy"), allow_pickle=True
        ).item()
...
def display_top_regions(state_counts, country_counts, top_n=10):
    ...
    for i, (state, count) in enumerate(state_counts.head(top_n).items(), 1):
        ...
        print(
            f"{i}. {Fore.GREEN}{state}{Style.RESET_ALL}: {count} users ({percent:.1f}%)"
        )
    ...
    for i, (country, count) in enumerate(country_counts.head(top_n).items(), 1):
        ...
        print(
            f"{i}. {Fore.GREEN}{country}{Style.RESET_ALL}: {count} users ({percent:.1f}%)"
        )
...
def display_recommendations(recommendations):
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        print(f"   {Fore.MAGENTA}Estimated Rating: {score:.2f}/10{Style.RESET_ALL}")
    ...
    while True:
        ...
        while True:
            try:
                ...
                if age < min_age or age > max_age:
                    print(
                        f"{Fore.YELLOW}Warning: Age outside the expected range. Using closest valid value.{Style.RESET_ALL}"
                    )
                    age = max(min_age, min(age, max_age))
                break
            except ValueError:
                print(
                    f"{Fore.RED}Please enter a valid number for age.{Style.RESET_ALL}"
                )
        ...
        try:
            recommendations = recommender.get_recommendations(
                user_features, num_results=NUM_RECOMMENDATIONS
            )
        ...
        continue_response = (
            input("\nWould you like to get more recommendations? (y/n): ")
            .strip()
            .lower()
        )
...
def main():
    ...
    if args.interactive or (
        args.age is None and args.state is None and args.country is None
    ):
        ...
        # Generate recommendations
        print(
            f"{Fore.BLUE}Generating recommendations for: Age {age}, {state}, {country}{Style.RESET_ALL}"
        )
        try:
