...
def test_model_storage(output_dir):
    ...
    assert os.path.exists(item_embeddings_path), f"Item embeddings not found at {item_embeddings_path}"
    assert os.path.exists(f"{ann_index_path}.faiss"), f"ANN index not found at {ann_index_path}.faiss"
    ...
    try:
        ...
        assert isinstance(user_model_state, dict), "User model is not a state dictionary"
        assert isinstance(item_model_state, dict), "Item model is not a state dictionary"
        
        # Check if state dicts have expected layer parameters
        assert any("weight" in key for key in user_model_state.keys()), "User model state dict missing expected weight parameters"
        assert any("weight" in key for key in item_model_state.keys()), "Item model state dict missing expected weight parameters"
        
        # Check if embeddings have the expected shape
        assert len(item_embeddings.shape) == 2, f"Item embeddings have unexpected shape: {item_embeddings.shape}"
